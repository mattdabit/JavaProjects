...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if(c < 1){
      ...
      newkid.parent = this.parent;
      ((SibTreeNode) child(children())).nextSibling = newkid;
      myTree.size++;
    } else {
      newkid.parent = this.parent;
      newkid.nextSibling = ((SibTreeNode) child(c)).nextSibling;
      ((SibTreeNode) child(c)).nextSibling = newkid;
    }
  }
