...
class Date {
  ...
  public Date(String s) {
      ...
      int[] num2 = {m, d, y};
      if (num == null || !isValidDate(num2[0], num2[1], num2[2])) {
  ...
  public boolean isAfter(Date d) {
      if (this.equals(d) || this.isBefore(d)) {
          return false;
      }
      return true;
  }
   ...
   *@param d the number that is compared.
   */
  public boolean equal(Date d) {
      if (d.day == this.day && d.year == this.year && d.month == this.month) {
          return true;
      }
      return false;
  }
  ...
  public int difference(Date d) {
      ...
      if (this.equal(d)) {
          ...
          for (int y = d.year + 1; y < this.year; y++) {
              Date dec = new Date(12, 31, y);
              dif += dec.dayInYear();
          }
          dif += this.dayInYear();
      } else if (this.isBefore(d)) {
          Date jan = new Date(1, 1, d.year);
          dif -= d.difference(jan);
          for (int y = d.year - 1; y > this.year; y--) {
              Date dec = new Date(12, 31, y);
              dif -= dec.dayInYear();
          }
          Date nwd = new Date(12, 31, this.year);
          dif -= nwd.difference(this) + 1;
      }
      return dif;
  }
