...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    vert = verticalSize;
    int[][] nwall = new int[(horiz - 1) * vert + (vert - 1) * horiz][];
    ...
    if (vert > 1) {
      ...
      for (j = 0; j < vert - 1; j++) {
        for (i = 0; i < horiz; i++) {
          hWalls[i][j] = true;
          int[] nHwall = new int[4];
          nHwall[0] = i;
          nHwall[1] = j;
          nHwall[2] = 0;
          nHwall[3] = i + j * horiz;
          nwall[numOfwall] = nHwall;
          numOfwall++;
        }
      }
    }
    ...
    if (horiz > 1) {
      ...
      for (i = 0; i < horiz - 1; i++) {
        for (j = 0; j < vert; j++) {
          vWalls[i][j] = true;
          int[] nVwall = new int[4];
          nVwall[0] = i;
          nVwall[1] = j;
          nVwall[2] = 1;
          nVwall[3] = i + j * horiz;
          nwall[numOfwall] = nVwall;
          numOfwall++;
        }
      }
    }
    ...
    int[] twall = new int[3];
    int rnum;
    DisjointSets aMaze = new DisjointSets(vert * horiz);
    for (int elem = nwall.length - 1; elem > 0; elem--){
      twall = nwall[elem];
      rnum = randInt(elem);
      nwall[elem] = nwall[rnum];
      nwall[rnum] = twall;
    }
    int x;
    int y;
    ...
    for(int q = 0; nwall.length > q; q++){
      ...
      xyc = x + y*horiz;
      x1y1c = x1 + y1 * horiz;
      if(aMaze.find(xyc) != aMaze.find(x1y1c) && x1y1c < horiz * vert){
        if(nwall[q][2] == 0){
          hWalls[nwall[q][0]][nwall[q][1]] = false;
        } else if(nwall[q][2] == 1){
          vWalls[nwall[q][0]][nwall[q][1]] = false;
        }
        aMaze.union(aMaze.find(xyc), aMaze.find(x1y1c));
      }
    }
